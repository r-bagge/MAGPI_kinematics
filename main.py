...
def stellar_gas_plots(field_name, cutoff, res_cutoff, n_ells, SNR_star, SNR_gas, mc=False, n=None, n_re=2):
    v_asym_ss = []
    v_asym_ss_err = []
    v_asym_gs = []
    v_asym_gs_err = []
    ...
    for f in range(len(csv_file)):
        ...
        if os.path.exists(gas_file) and os.path.exists(star_file):
            ...
            x0 = int(x0 / 2)
            y0 = int(y0 / 2)
            print(f"Max SNR = {clip:.2f}...\n")
            log_file.write(f"Max SNR = {clip:.2f}...\n")
            if clip < SNR_star:
            ...
            if ha_check < 50:
                ...
                max_line = pd.read_csv("MAGPI_Emission_Max_Line.csv")
                ...
                if mc == True and n == None:
                    print("Need number of MCs idiot")
                    break
                if mc and n != None:
                    v_asym_tot_mc = np.zeros(n)
                    for h in range(n):
                        ...
                        k = kinemetry(img=model_s, x0=x0, y0=y0, ntrm=11, plot=False, verbose=False, radius=rad,
                                      bmodel=True, rangePA=[-120, 120], rangeQ=[q[f] - 0.1, q[f] + 0.1], allterms=True,
                                      fixcen=True)
                        ...
                        try:
                            v_asym_tot_mc[h] = np.nansum(k_flux_s_k0 * ((k2 + k3 + k4 + k5) / (4 * k1))) / np.nansum(k_flux_s_k0)
                        except ValueError:
                            continue
                    ...
                    v_asym_ss.append(np.nanmean(v_asym_tot_mc))
                    v_asym_ss_err.append(np.nanstd(v_asym_tot_mc))
                    pa_gs.append(np.nan)
                    pa_ss.append(pa_s)
                    d_pas.append(np.nan)
                ...
                if ha_check > bl_check:
                    ...
                    out.writeto("plots/"+field_name + "/fits_file/" + str(file_name[f]) + "_stellar_kinemetry.fits", overwrite=True)
                else:
                    s_flux, s_velo, s_velo_err, s_sigma = starfile[7].data, starfile[1].data, starfile[3].data, \
                                                          starfile[4].data
                    g_flux, g_flux_err, g_velo, g_velo_err, g_sigma = gasfile[bright_line].data, gasfile[bright_line_err].data, gasfile[
                    ...
                    out = fits.HDUList([hdu0, hdu1, hdu2, hdu3, hdu4, hdu5, hdu6, hdu7, hdu8, hdu9, hdu10])
                    out.writeto("plots/"+field_name + "/fits_files/" + str(file_name[f]) + "_stellar_kinemetry.fits", overwrite=True)
                ...
                if mc == True and n == None:
                    ...
                    break
                if mc and n != None:
                    v_asym_tot_mc_g = np.zeros(n)
                    v_asym_tot_mc_s = np.zeros(n)
                    for h in range(n):
                        print(f"{h + 1} Monte Carlo iteration...\n")
                        model_s = ks.velkin
                        model_g = kg.velkin
                        ...
                        kg_mc = kinemetry(img=model_g, x0=x0, y0=y0, ntrm=11, plot=False, verbose=False, radius=rad,
                                      bmodel=True, rangePA=[-120, 120], rangeQ=[q[f] - 0.1, q[f] + 0.1], allterms=True,
                                      fixcen=True)
                        ...
                        kg1 = np.sqrt(kg_mc.cf[:, 1] ** 2 + kg_mc.cf[:, 2] ** 2)
                        kg2 = np.sqrt(kg_mc.cf[:, 3] ** 2 + kg_mc.cf[:, 4] ** 2)
                        kg3 = np.sqrt(kg_mc.cf[:, 3] ** 2 + kg_mc.cf[:, 4] ** 2)
                        kg4 = np.sqrt(kg_mc.cf[:, 7] ** 2 + kg_mc.cf[:, 8] ** 2)
                        kg5 = np.sqrt(kg_mc.cf[:, 5] ** 2 + kg_mc.cf[:, 6] ** 2)
                        v_asym_g = (kg2 + kg3 + kg4 + kg5) / (4 * kg1)

                        v_asym_tot_mc_g[h] = np.nansum(k_flux_g_k0 * v_asym_g) / np.nansum(k_flux_g_k0)
                        v_asym_tot_mc_s[h] = np.nansum(k_flux_s_k0 * v_asym_s) / np.nansum(k_flux_s_k0)

                    galaxies.append(file_name[f])
                    v_asym_gs.append(np.nanmean(v_asym_tot_mc_g))
                    v_asym_gs_err.append(np.nanstd(v_asym_tot_mc_g))
                    v_asym_ss.append(np.nanmean(v_asym_tot_mc_s))
                    v_asym_ss_err.append(np.nanstd(v_asym_tot_mc_s))
                    pa_gs.append(pa_g)
                    pa_ss.append(pa_s)
                    d_pas.append(d_PA)
                    v_rot_g.append(np.nanmax(kg1))
            ...
            gasfile = fits.open(gas_file)
            g_flux, g_flux_err, g_velo, g_velo_err, g_sigma = gasfile[49].data, gasfile[50].data, gasfile[9].data, gasfile[10].data,\
                                                  gasfile[11].data
            g_velo = clean_images(g_velo, pa[f], r50[f], r50[f] * q[f], img_err=g_flux / g_flux_err)
            g_velo_err = clean_images(g_velo_err, pa[f], r50[f], r50[f] * q[f], img_err=g_flux / g_flux_err)
            g_sigma = clean_images(g_sigma, pa[f], r50[f], r50[f] * q[f], img_err=g_flux / g_flux_err)
            ...
            if ha_check < 50:
                ...
                if mc and n != None:
                    ...
                    for h in range(n):
                        ...
                        k5 = np.sqrt(k.cf[:, 9] ** 2 + k.cf[:, 10] ** 2)
                        try:
                            v_asym_tot_mc[h] = np.nansum(k_flux_g_k0 * ((k2 + k3 + k4 + k5) / (4 * k1))) / np.nansum(
                                k_flux_g_k0)
                        except ValueError:
                            continue
...
for i in range(len(fields)):
    ...
    galaxy, v_asym_g, v_asym_g_err, v_asym_s,v_asym_s_err, pa_g, pa_s, d_pa, vrot_g, vrot_s = stellar_gas_plots(magpi, cutoff=1,
                                                                                                res_cutoff=res_cutoff,
                                                                                                n_ells=5, SNR_gas=20,
                                                                                                SNR_star=3,
                                                                                                mc=True, n=100, n_re=2)
...
# print(len(new_v_asym_g_errs))
# print(len(new_v_asym_s_errs))
# print(len(new_v_asym_gs))
