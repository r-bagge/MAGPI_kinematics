import numpy as np
import matplotlib.pyplot as plt
from matplotlib.patches import Circle
from matplotlib.patches import Ellipse
from astropy.io import fits
import astropy.units as u
import os
import shutil
import pandas as pd
from astropy.cosmology import Planck18 as cosmo
import glob
from kinemetry import kinemetry
import cmasher as cmr
from dust_extinction.parameter_averages import CCM89


def gas_kinemetry(galaxy,):

    return galaxy, v_asym_ss


def clean_images(img, pa, a, b, img_err=None):
    y0, x0 = img.shape
    y0, x0 = y0 / 2, x0 / 2
    pa = pa - 90
    pa = np.radians(pa)
    for i in range(len(img[:, 0])):
        for j in range(len(img[0, :])):
            side1 = (((j - x0) * np.cos(pa)) + ((i - y0) * np.sin(pa))) ** 2 / (a ** 2)
            side2 = (((j - x0) * np.sin(pa)) - ((i - y0) * np.cos(pa))) ** 2 / (b ** 2)
            if side1 + side2 > 8:
                img[i, j] = np.nan
            if img_err is not None and abs(img_err[i, j]) < 3:
                img[i, j] = np.nan
    return img


def MAGPI_kinemetry(source_cat, n=100, n_ells=5, n_re=2, SNR_Star=3, SNR_Gas=20):
    gal_id = []
    v_asym_ss = []
    v_asym_gs = []
    v_asym_gs_err = []
    v_asym_ss_err = []
    v_rot_g = []
    v_rot_s = []
    keven_g = []
    keven_s = []
    kodd_g = []
    kodd_s = []
    pa_ss = []
    pa_gs = []
    d_pas = []
    logfile = open("plots/MAGPI_kinemetry.txt", "w")
    master = pd.read_csv(source_cat,skiprows=16)
    z = master["z"].to_numpy()
    r50 = master["R50_it"].to_numpy() / 0.2
    q = master["axrat_it"].to_numpy()
    pa = master["ang_it"].to_numpy()
    quality = master["QOP"].to_numpy()
    galaxy = master["MAGPIID"].to_numpy()
    print(len(galaxy))
    DL = cosmo.luminosity_distance(z).to(u.kpc).value
    pix = np.radians(0.33 / 3600) * DL
    res_cutoff = (0.65/2)/0.2
    cutoff = 1
    for i in range(len(master)):
        if z[i] > 0.35:
            print(f"MAGPIID = {galaxy[i]}, z = {z[i]:.3f}, Redshift not in range!")
            logfile.write(f"MAGPIID = {galaxy[i]}, z = {z[i]:.3f}, Redshift not in range!\n")
            continue
        elif z[i] < 0.28:
            print(f"MAGPIID = {galaxy[i]}, z = {z[i]:.3f}, Redshift not in range!")
            logfile.write(f"MAGPIID = {galaxy[i]}, z = {z[i]:.3f}, Redshift not in range!\n")
            continue
        elif quality[i] < 3:
            print(f"MAGPIID = {galaxy[i]}, z = {z[i]:.3f}, Redshift failed QC check!")
            logfile.write(f"MAGPIID = {galaxy[i]}, z = {z[i]:.3f}, Redshift failed QC check!\n")
            continue
        elif r50[i] < cutoff * res_cutoff:
            print(f"MAGPIID = {galaxy[i]}, r50 = {r50[i]:.2f} pix, not resolved enough!")
            logfile.write(f"MAGPIID = {galaxy[i]}, r50 = {r50[i]:.2f} pix, not resolved enough!\n")
            continue
        elif galaxy[i] == int("1207128248") or galaxy[i] == int("1506117050"):
            print(f"MAGPIID = {galaxy[i]}, fixing PA\n")
            logfile.write(f"MAGPIID = {galaxy[i]}, fixing PA\n")
            pa[i] = pa[i] - 90
        elif galaxy[i] == int("1207197197"):
            print(f"MAGPIID = {galaxy[i]}, fixing PA\n")
            logfile.write(f"MAGPIID = {galaxy[i]}, fixing PA\n")
            pa[i] = pa[i] - 180
        elif galaxy[i] == int("1204192193"):
            print(f"MAGPIID = {galaxy[i]}, For Qainhui\n")
            logfile.write(f"MAGPIID = {galaxy[i]}, For Qainhui\n")
        else:
            print(f"MAGPIID = {galaxy[i]}, z = {z[i]:.3f}, Redshift passed!")
            print(f"MAGPIID = {galaxy[i]}, r50 = {r50[i]:.3f}, Res. passed!")
            print(f"MAGPIID = {galaxy[i]} is {(r50[i] / res_cutoff):.3f} beam elements!")
            logfile.write(f"MAGPIID = {galaxy[i]}, z = {z[i]:.3f}, Redshift passed!\n")
            logfile.write(f"MAGPIID = {galaxy[i]}, r50 = {r50[i]:.3f}, Res. passed!\n")
            logfile.write(f"MAGPIID = {galaxy[i]} is {(r50[i] / res_cutoff):.3f} beam elements!\n")
        field = str(galaxy[i])[:4]
        star_file = "MAGPI_Absorption_Lines/MAGPI" + field + "/galaxies/" + str(galaxy[i]) + "_kinematics_ppxf-maps.fits"
        gas_file = "MAGPI_Emission_Lines/MAGPI" + field + "/" + field + "_v2.2.1_GIST_EmissionLine_Maps/MAGPI" + str(galaxy[i]) + "_GIST_EmissionLines.fits"

        if os.path.exists(star_file):
            star_file_catch = True
        else:
            print("No stellar kinematics!")
            logfile.write("No stellar kinematics!\n")
            star_file_catch = False

        if os.path.exists(gas_file):
            gas_file_catch = True
        else:
            print("No gas kinematics!")
            logfile.write("No gas kinematics!\n")
            gas_file_catch = False

        # Check to see if there is neither gas or star data
        if star_file_catch and gas_file_catch:
            print("No kinematics! Skipping "+galaxy+"!")
            logfile.write("No kinematics! Skipping "+galaxy+"!\n")
            continue

        if star_file_catch==False and gas_file_catch:
            print("No stellar kinematics! Doing gas kinemetry only!")
            print("No stellar kinematics! Doing gas kinemetry only!\n", file=logfile)
            gasfile = fits.open(gas_file)
            g_flux, g_flux_err, g_velo, g_velo_err, g_sigma = gasfile[49].data, gasfile[50].data, gasfile[9].data, gasfile[10].data, gasfile[11].data
            gasfile.close()
            g_velo = clean_images(g_velo, pa[i], r50[i], r50[i] * q[i], img_err=g_flux / g_flux_err)
            g_velo_err = clean_images(g_velo_err, pa[i], r50[i], r50[i] * q[i], img_err=g_flux / g_flux_err)
            g_sigma = clean_images(g_sigma, pa[i], r50[i], r50[i] * q[i], img_err=g_flux / g_flux_err)
            g_flux = clean_images(g_flux, pa[i], r50[i], r50[i] * q[i], img_err=g_flux / g_flux_err)
            g_flux = g_flux / g_flux_err

            clip = np.nanmax(g_flux)
            y0, x0 = g_flux.shape
            x0 = int(x0 / 2)
            y0 = int(y0 / 2)
            print(f"Max Gas SNR = {clip:.2f}...")
            print(f"Max Gas SNR = {clip:.2f}...\n", file=logfile)
            if clip < SNR_Gas:
                print("Not Plotting or doing Kinemetry on " + str(galaxy[i]) + " because its heinous looking\n")
                logfile.write(
                    "Not Plotting or doing Kinemetry on " + str(galaxy[i]) + " because its heinous looking\n")
                continue
            elif np.isinf(clip) or np.isnan(clip):
                print("Not Plotting or doing Kinemetry on " + str(galaxy[i]) + " because its heinous looking\n")
                logfile.write(
                    "Not Plotting or doing Kinemetry on " + str(galaxy[i]) + " because its heinous looking\n")
                continue
            start = (0.65 / 2) / 0.2
            step = (0.65 / 2) / 0.2
            end = n_re * r50[i]
            rad = np.arange(start, end, step)
            if len(rad) < n_ells:
                print("Not enough ellipses!")
                logfile.write("Not enough ellipses!\n")
                continue
            print("Doing kinemetry on stars and gas!")
            print("Doing kinemetry on stars and gas!\n", file=logfile)
            g_velo[np.isnan(g_velo)] = 0
            g_velo_err[np.isnan(g_velo_err)] = 0
            g_flux[np.isnan(g_flux)] = 0

            kg = kinemetry(img=g_velo, x0=x0, y0=y0, ntrm=11, plot=False, verbose=False, radius=rad,
                           bmodel=True, rangePA=[-120, 120], rangeQ=[q[i] - 0.1, q[i] + 0.1], allterms=True,
                           cover=0.95)
            kg1 = np.sqrt(kg.cf[:, 1] ** 2 + kg.cf[:, 2] ** 2)
            v_rot_g.append(np.nanmax(kg1))
            k_flux_g = kinemetry(img=g_flux, x0=x0, y0=y0, ntrm=10, plot=False, verbose=False, radius=rad,
                                 bmodel=True,
                                 rangePA=[pa[i] - 10, pa[i] + 10], rangeQ=[q[i] - 0.1, q[i] + 0.1], allterms=True,
                                 cover=0.95)
            kg_flux_k0 = k_flux_g.cf[:,0]
            pa_g = np.nanmedian(kg.pa)
            pa_s = np.nanmedian(np.nan)
            d_pa = np.abs(np.nan)
            v_asym_g_mc = np.zeros(n)
            for h in range(n):
                print(f"{h + 1} Monte Carlo iteration...")
                model_g = kg.velkin
                for i in range(len(g_velo[:, 0])):
                    for j in range(len(g_velo[0, :])):
                        model_g[i, j] = + np.random.normal(loc=g_velo[i, j], scale=g_velo_err[i, j])
                kg = kinemetry(img=model_g, x0=x0, y0=y0, ntrm=11, plot=False, verbose=False, radius=rad,
                               bmodel=True, rangePA=[-120, 120], rangeQ=[q[i] - 0.1, q[i] + 0.1], allterms=True,
                               fixcen=True)
                kg1 = np.sqrt(kg.cf[:, 1] ** 2 + kg.cf[:, 2] ** 2)
                kg2 = np.sqrt(kg.cf[:, 3] ** 2 + kg.cf[:, 4] ** 2)
                kg3 = np.sqrt(kg.cf[:, 3] ** 2 + kg.cf[:, 4] ** 2)
                kg4 = np.sqrt(kg.cf[:, 7] ** 2 + kg.cf[:, 8] ** 2)
                kg5 = np.sqrt(kg.cf[:, 5] ** 2 + kg.cf[:, 6] ** 2)
                try:
                    v_asym_g_mc[h] = np.nansum(kg_flux_k0 * ((kg2 + kg3 + kg4 + kg5) / (4 * kg1))) / np.nansum(
                        kg_flux_k0)
                except ValueError:
                    v_asym_g_mc[h] = np.nan
            print(f"Finished Monte Carlo!")
            print(f"Finished Monte Carlo!\n",file=logfile)
            gal_id.append(galaxy)
            v_asym_gs.append(np.nanmean(v_asym_g_mc))
            v_asym_gs_err.append(np.nanstd(v_asym_g_mc))
            v_asym_ss.append(np.nan)
            v_asym_ss_err.append(np.nan)
            pa_gs.append(pa_g)
            pa_ss.append(pa_s)
            d_pas.append(d_pa)
            v_rot_s.append(np.nan)
        # Stellar kinemetry
        if star_file_catch and gas_file_catch==False:
            print("No gas kinematics! Doing stellar kinemtry only!")
            print("No gas kinematics! Doing stellar kinemtry only!\n", file=logfile)
            starfile = fits.open(star_file)
            s_flux, s_velo, s_velo_err, s_sigma = starfile[7].data, starfile[1].data, starfile[3].data, starfile[4].data
            starfile.close()

            clip = np.nanmax(s_flux)
            y0, x0 = s_flux.shape
            x0 = int(x0 / 2)
            y0 = int(y0 / 2)
            print(f"Max Stellar SNR = {clip:.2f}...")
            print(f"Max Stellar SNR = {clip:.2f}...\n", file=logfile)
            if clip < SNR_Star:
                print("Not Plotting or doing Kinemetry on " + str(galaxy[i]) + " because its heinous looking\n")
                logfile.write(
                    "Not Plotting or doing Kinemetry on " + str(galaxy[i]) + " because its heinous looking\n")
                continue
            elif np.isinf(clip) or np.isnan(clip):
                print("Not Plotting or doing Kinemetry on " + str(galaxy[i]) + " because its heinous looking\n")
                logfile.write(
                    "Not Plotting or doing Kinemetry on " + str(galaxy[i]) + " because its heinous looking\n")
                continue
            start = (0.65 / 2) / 0.2
            step = (0.65 / 2) / 0.2
            end = n_re * r50[i]
            rad = np.arange(start, end, step)
            if len(rad) < n_ells:
                print("Not enough ellipses!")
                logfile.write("Not enough ellipses!\n")
                continue
            print("Doing kinemetry on stars and gas!")
            print("Doing kinemetry on stars and gas!\n", file=logfile)
            s_velo[np.isnan(s_velo)] = 0
            s_velo_err[np.isnan(s_velo_err)] = 0
            s_flux[np.isnan(s_flux)] = 0

            ks = kinemetry(img=s_velo, x0=x0, y0=y0, ntrm=11, plot=False, verbose=False, radius=rad,
                           bmodel=True, rangePA=[-120, 120], rangeQ=[q[i] - 0.1, q[i] + 0.1], allterms=True,
                           cover=0.95)
            ks1 = np.sqrt(ks.cf[:,1]**2 + ks.cf[:,2]**2)
            v_rot_s.append(np.nanmax(ks1))
            k_flux_s = kinemetry(img=s_flux, x0=x0, y0=y0, ntrm=10, plot=False, verbose=False, radius=rad,
                                 bmodel=True,
                                 rangePA=[pa[i] - 10, pa[i] + 10], rangeQ=[q[i] - 0.1, q[i] + 0.1], allterms=True,
                                 cover=0.95)
            ks_flux_k0 = k_flux_s.cf[:,0]
            pa_g = np.nanmedian(np.nan)
            pa_s = np.nanmedian(ks.pa)
            d_pa = np.abs(np.nan)
            v_asym_s_mc = np.zeros(n)
            for h in range(n):
                print(f"{h + 1} Monte Carlo iteration...")
                model_s = ks.velkin
                for i in range(len(s_velo[:, 0])):
                    for j in range(len(s_velo[0, :])):
                        model_s[i, j] = + np.random.normal(loc=s_velo[i, j], scale=s_velo_err[i, j])
                ks = kinemetry(img=model_s, x0=x0, y0=y0, ntrm=11, plot=False, verbose=False, radius=rad,
                               bmodel=True, rangePA=[-120, 120], rangeQ=[q[i] - 0.1, q[i] + 0.1], allterms=True,
                               fixcen=True)
                ks1 = np.sqrt(ks.cf[:, 1] ** 2 + ks.cf[:, 2] ** 2)
                ks2 = np.sqrt(ks.cf[:, 3] ** 2 + ks.cf[:, 4] ** 2)
                ks3 = np.sqrt(ks.cf[:, 3] ** 2 + ks.cf[:, 4] ** 2)
                ks4 = np.sqrt(ks.cf[:, 7] ** 2 + ks.cf[:, 8] ** 2)
                ks5 = np.sqrt(ks.cf[:, 5] ** 2 + ks.cf[:, 6] ** 2)
                v_asym = (ks2 + ks3 + ks4 + ks5) / (4 * ks1)
                try:
                    v_asym_s_mc[h] = np.nansum(ks_flux_k0 * v_asym/np.nansum(ks_flux_k0))
                except ValueError:
                    v_asym_s_mc[h] = np.nan
            print(f"Finished Monte Carlo!")
            print(f"Finished Monte Carlo!\n",file=logfile)
            gal_id.append(galaxy)
            v_asym_gs.append(np.nan)
            v_asym_gs_err.append(np.nan)
            v_asym_ss.append(np.nanmean(v_asym_s_mc))
            v_asym_ss_err.append(np.nanstd(v_asym_s_mc))
            pa_gs.append(pa_g)
            pa_ss.append(pa_s)
            d_pas.append(d_pa)
            v_rot_g.append(np.nan)
            
        if star_file_catch and gas_file_catch:
            print("Doing kinemetry on the stars and the gas!")
            print("Doing kinemetry on the stars and gas!\n",file=logfile)
            starfile = fits.open(star_file)
            gasfile = fits.open(gas_file)
            s_flux, s_velo, s_velo_err, s_sigma = starfile[7].data, starfile[1].data, starfile[3].data, starfile[4].data
            starfile.close()
            g_flux, g_flux_err, g_velo, g_velo_err, g_sigma = gasfile[49].data, gasfile[50].data, gasfile[9].data, \
                                                              gasfile[10].data, gasfile[11].data
            gasfile.close()
            g_velo = clean_images(g_velo, pa[i], r50[i], r50[i] * q[i], img_err=g_flux / g_flux_err)
            g_velo_err = clean_images(g_velo_err, pa[i], r50[i], r50[i] * q[i], img_err=g_flux / g_flux_err)
            g_sigma = clean_images(g_sigma, pa[i], r50[i], r50[i] * q[i], img_err=g_flux / g_flux_err)
            g_flux = clean_images(g_flux, pa[i], r50[i], r50[i] * q[i], img_err=g_flux / g_flux_err)
            g_flux = g_flux / g_flux_err

            clip = np.nanmax(s_flux)
            y0, x0 = s_flux.shape
            x0 = int(x0 / 2)
            y0 = int(y0 / 2)
            print(f"Max Stellar SNR = {clip:.2f}...")
            logfile.write(f"Max Stellar SNR = {clip:.2f}...\n")
            if clip < SNR_Star:
                print("Not Plotting or doing Kinemetry on " + str(galaxy[i]) + " because its stars are heinous looking\n")
                logfile.write(
                    "Not Plotting or doing Kinemetry on " + str(galaxy[i]) + " because its stars heinous looking\n")
                continue
            elif np.isinf(clip) or np.isnan(clip):
                print("Not Plotting or doing Kinemetry on " + str(galaxy[i]) + " because its stars heinous looking\n")
                logfile.write(
                    "Not Plotting or doing Kinemetry on " + str(galaxy[i]) + " because its stars heinous looking\n")
                continue
            if clip < SNR_Gas:
                print("Not Plotting or doing Kinemetry on " + str(galaxy[i]) + " because its gas is heinous looking\n")
                logfile.write(
                    "Not Plotting or doing Kinemetry on " + str(galaxy[i]) + " because its gas is heinous looking\n")
                continue
            elif np.isinf(clip) or np.isnan(clip):
                print("Not Plotting or doing Kinemetry on " + str(galaxy[i]) + " because its gas is heinous looking\n")
                logfile.write(
                    "Not Plotting or doing Kinemetry on " + str(galaxy[i]) + " because its gas is heinous looking\n")
                continue

            start = (0.65 / 2) / 0.2
            step = (0.65 / 2) / 0.2
            end = n_re * r50[i]
            rad = np.arange(start, end, step)
            if len(rad) < n_ells:
                print("Not enough ellipses!")
                logfile.write("Not enough ellipses!\n")
                continue
            s_velo[np.isnan(s_velo)] = 0
            g_velo[np.isnan(g_velo)] = 0
            g_velo_err[np.isnan(g_velo_err)] = 0
            g_flux[np.isnan(g_flux)] = 0
            s_flux[np.isnan(s_flux)] = 0

            print("Doing kinemetry on stars and gas!")

            ks = kinemetry(img=s_velo, x0=x0, y0=y0, ntrm=11, plot=False, verbose=False, radius=rad,
                           bmodel=True, rangePA=[-120, 120], rangeQ=[q[i] - 0.1, q[i] + 0.1], allterms=True,
                           cover=0.95)
            ks1 = np.sqrt(ks.cf[:,1]**2+ks.cf[:,2]**2)
            v_rot_s.append(np.nanmax(ks1))
            kg = kinemetry(img=g_velo, x0=x0, y0=y0, ntrm=11, plot=False, verbose=False, radius=rad,
                           bmodel=True, rangePA=[-120, 120], rangeQ=[q[i] - 0.1, q[i] + 0.1], allterms=True,
                           cover=0.95)
            kg1 = np.sqrt(kg.cf[:, 1] ** 2 + kg.cf[:, 2] ** 2)
            v_rot_g.append(np.nanmax(kg1))
            k_flux_g = kinemetry(img=g_flux, x0=x0, y0=y0, ntrm=10, plot=False, verbose=False, radius=rad,
                                 bmodel=True,
                                 rangePA=[pa[i] - 10, pa[i] + 10], rangeQ=[q[i] - 0.1, q[i] + 0.1], allterms=True,
                                 cover=0.95)
            k_flux_s = kinemetry(img=s_flux, x0=x0, y0=y0, ntrm=10, plot=False, verbose=False, radius=rad,
                                 bmodel=True,
                                 rangePA=[pa[i] - 10, pa[i] + 10], rangeQ=[q[i] - 0.1, q[i] + 0.1], allterms=True,
                                 cover=0.95)
            ks_flux_k0 = k_flux_s.cf[:, 0]
            kg_flux_k0 = k_flux_g.cf[:, 0]
            pa_g = np.nanmedian(kg.pa)
            pa_s = np.nanmedian(ks.pa)
            d_pa = np.abs(pa_g - pa_s)
            v_asym_s_mc = np.zeros(n)
            v_asym_g_mc = np.zeros(n)
            for h in range(n):
                print(f"{h + 1} Monte Carlo iteration...")
                model_s = ks.velkin
                model_g = kg.velkin
                for i in range(len(s_velo[:, 0])):
                    for j in range(len(s_velo[0, :])):
                        model_s[i, j] = + np.random.normal(loc=s_velo[i, j], scale=s_velo_err[i, j])
                        model_g[i, j] = + np.random.normal(loc=g_velo[i, j], scale=g_velo_err[i, j])
                ks = kinemetry(img=model_s, x0=x0, y0=y0, ntrm=11, plot=False, verbose=False, radius=rad,
                               bmodel=True, rangePA=[-120, 120], rangeQ=[q[i] - 0.1, q[i] + 0.1], allterms=True,
                               fixcen=True)
                ks1 = np.sqrt(ks.cf[:, 1] ** 2 + ks.cf[:, 2] ** 2)
                ks2 = np.sqrt(ks.cf[:, 3] ** 2 + ks.cf[:, 4] ** 2)
                ks3 = np.sqrt(ks.cf[:, 3] ** 2 + ks.cf[:, 4] ** 2)
                ks4 = np.sqrt(ks.cf[:, 7] ** 2 + ks.cf[:, 8] ** 2)
                ks5 = np.sqrt(ks.cf[:, 5] ** 2 + ks.cf[:, 6] ** 2)
                v_asym = (ks2 + ks3 + ks4 + ks5) / (4 * ks1)
                try:
                    v_asym_s_mc[h] = np.nansum(ks_flux_k0 * v_asym / np.nansum(ks_flux_k0))
                except ValueError:
                    v_asym_s_mc[h] = np.nan

                kg = kinemetry(img=model_g, x0=x0, y0=y0, ntrm=11, plot=False, verbose=False, radius=rad,
                               bmodel=True, rangePA=[-120, 120], rangeQ=[q[i] - 0.1, q[i] + 0.1], allterms=True,
                               fixcen=True)
                kg1 = np.sqrt(kg.cf[:, 1] ** 2 + kg.cf[:, 2] ** 2)
                kg2 = np.sqrt(kg.cf[:, 3] ** 2 + kg.cf[:, 4] ** 2)
                kg3 = np.sqrt(kg.cf[:, 3] ** 2 + kg.cf[:, 4] ** 2)
                kg4 = np.sqrt(kg.cf[:, 7] ** 2 + kg.cf[:, 8] ** 2)
                kg5 = np.sqrt(kg.cf[:, 5] ** 2 + kg.cf[:, 6] ** 2)
                try:
                    v_asym_g_mc[h] = np.nansum(kg_flux_k0 * ((kg2 + kg3 + kg4 + kg5) / (4 * kg1))) / np.nansum(
                        kg_flux_k0)
                except ValueError:
                    v_asym_g_mc[h] = np.nan
            print(f"Finished Monte Carlo!")
            print(f"Finished Monte Carlo!\n", file=logfile)
            gal_id.append(galaxy)
            v_asym_gs.append(np.nanmean(v_asym_g_mc))
            v_asym_gs_err.append(np.nanstd(v_asym_g_mc))
            v_asym_ss.append(np.nanmean(v_asym_s_mc))
            v_asym_ss_err.append(np.nanstd(v_asym_s_mc))
            pa_gs.append(pa_g)
            pa_ss.append(pa_s)
            d_pas.append(d_pa)

    for ls in [gal_id,v_asym_gs,v_asym_gs_err,v_asym_ss,v_asym_ss_err,v_asym_ss_err,pa_gs,pa_ss,d_pas,v_rot_g,v_rot_s]:
        ls = np.array(ls)

    return gal_id,v_asym_gs,v_asym_gs_err,v_asym_ss,v_asym_ss_err,pa_gs,pa_ss,d_pas,v_rot_g,v_rot_s


gal_id,v_asym_gs,v_asym_gs_err,v_asym_ss,v_asym_ss_err,pa_gs,pa_ss,d_pas,v_rot_g,v_rot_s = MAGPI_kinemetry(source_cat="MAGPI_csv/MAGPI_master_source_catalogue.csv",
                                                                                                           n=100,
                                                                                                           n_ells=5,
                                                                                                           n_re=2,
                                                                                                           SNR_Star=3,
                                                                                                           SNR_Gas=20)

df = pd.DataFrame({"MAGPIID":gal_id,
                   "v_asym_g":v_asym_gs,
                   "v_asym_g_err":v_asym_gs_err,
                   "v_asym_s":v_asym_ss,
                   "v_asym_s_err":v_asym_ss_err,
                   "PA_g":pa_gs,
                   "PA_s":pa_ss,
                   "D_PA":d_pas,
                   "V_rot_g":v_rot_g,
                   "V_rot_s":v_rot_s})
df.to_csv("MAPGI_kinemetry_sample_mc.csv")


            
            


            





